...
xmlstarlet ed --inplace -u //config/hostname -v mysql -u //config/user -v koha_knihovna -u //config/pass -v $KOHA_PASSWORD /etc/koha/sites/knihovna/koha-conf.xml 

#TODO: wait for db start...better
sleep 15
koha_db_count=`echo "use koha_knihovna; show tables;" | mysql --host=mysql --user=root --password="$KOHA_PASSWORD" | wc -l`

if [ "$koha_db_count" -lt 1 ]
then
   echo "Koha database is missing"
   mysql  -p koha_knihovna --user=koha_knihovna --host=mysql --password=$KOHA_PASSWORD < /import.sql
